<template>
  <div class="helloFirst">
    <div class="margin flex seachI">
      <input type="file" style="display: none" ref="upImg" @change="changeImg($event,1)" :multiple="false">
      <el-button type="primary" round icon="el-icon-upload2" style="width:160px;" @click="upLoadimg()">上传</el-button>
      <span @click="hrefL()">或</span>
    ...
    <div class="margin" ref="contentImg">
      <div class="Uploading flex a-i" v-show="upLoading">
        ...
        <i class="el-icon-loading" style="font-size: 42px;color: #3a8ee6;" v-show="!imageMsg.queueNumber"></i>
        <div v-show="imageMsg.queueNumber && !imageMsg.status">
          <i class="el-icon-s-flag" style="font-size: 40px;color: #a74d45;"></i>
          当前正在排队,请稍后...
          <p style="margin-top: 20px;text-align: center">
            排队位置{{imageMsg.queueNumber}}
          </p>
        </div>
        <div v-show="imageMsg.queueNumber && imageMsg.status && imageMsg.msg" style="text-align: center">
          <i class="el-icon-error" style="font-size: 40px;color: #a74d45;"></i>
          <p style="margin-top: 20px;text-align: center;">
            {{imageMsg.msg}} <a style="color: red;" href="http://www.picup.shop/loginOrRegister.html#/" target="_blank">前往登录或注册</a>
          </p>
      ...
      <div class=" flex j-b content" v-show="canvasShow && this.files.length<2">
        ...
        <!--        <div class="swich">-->
        <!--          <el-switch-->
        <!--            v-model="leftwhichcanvas"-->
        <!--            active-text="拖拽缩放模式"-->
        <!--            inactive-text="擦除还原模式"-->
        <!--            @change="changmoshi()">-->
        <!--          </el-switch>-->
        <!--        </div>-->
        <div class="left">
        ...
        <div class="right">
          ...
          <p style="width:90%;font-size: 12px;text-align: center;margin-top: 50px">发丝效果不好？试试 <a
            href="http://www.picup.shop/people.html" style="color:rgb(52,165,242);">人像抠图</a></p>
          <el-button type="primary" round icon="el-icon-download" style="width:160px;margin:15px 0 0 150px;"
                     @click="save()">下载
          </el-button>
          <p style="width:90%;font-size: 12px;text-align: center;margin-top: 15px">下载高清大图请去 <a
            href="http://www.picup.shop" style="color:rgb(52,165,242);">皮卡智能PicUP.AI</a></p>
      ...
      <div class="moreImgSet flex j-b" v-show="canvasShow && this.files.length>1">
        ...
        <div class="moveLeft">
          ...
          <el-button round size="small" style="width: 100px;display: block;margin: 30px auto 0;" icon="el-icon-plus"
                     @click="upLoadimg()">批量抠图
          </el-button>
        </div>
        <div class="moveright">
          <p>去背景图片</p>
          ...
          <el-button type="primary" round icon="el-icon-download" style="width: 150px;display: block;margin: 0 auto 0;"
                     size="small" :disabled="leftMove.length!==files.length"
                     @click="saveMove()">下载
          </el-button>
...
<script>
    ...
    import {copyUpload, uploadImgApi, getMattingInfo} from "../apis/index";
    ...
    export default {
        ...
        methods: {
            ...
            copyImgUrl() {//粘贴图片链接确定
                ...
                this.toscroll()
                let data = {url: this.imgUrl,'mattingType': 6};
                copyUpload( data ).then( res => {
                    ...
                    if (res.code == 0) {
                        ...
                        this.imageMsg={status:1,msg:res.msg,queueNumber:1}
                    }
                } ).catch( err => {
                    if (_self.timer) {
                    ...
                    if (window.FileReader) {
                        ...
                        reader.onloadend = function (e) {
                            ...
                            uploadImgApi( param ).then( res => {
                                ...
                                if (res.code == 0) {
                                    if (res.data.status !== 'success') {
                                        _self.getMattingInfoss( e.target.result, res.data.fileId, val )
                                        return
                                    }
                                    let obj = {
            ...
            getMattingInfoss(or, id, val) {//getMattingInfo封装
                getMattingInfo( {fileId: id} ).then( res => {
                    if (res.data.status === 'success') {
                        let obj = {
                            name: val.name.substring( 0, val.name.lastIndexOf( '.' ) ),
                            img: res.data.bgRemovedPreview,
                        ...
                        this.leftMove.push( or )
                        this.rightMove.push( obj )
                    } else {
                        setTimeout( () => {
                            this.getMattingInfoss( or, id, val )
                        }, 2000 )
                    }
            ...
            setImgInitialization(file, e) {//图片上传接口
                ...
                uploadImgApi( param ).then( res => {
                    if (res.code == 0) {
                        ...
                        _self.$message( {message: res.msg, type: 'warning'} );
                        this.imageMsg={status:1,msg:res.msg,queueNumber:1}
                    }
                } ).catch( err => {
                    _self.$refs.upImg.value = ''
